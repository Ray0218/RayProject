...
- (void)setUpProperties
{
    ...
    if ([[UIDevice currentDevice].systemVersion floatValue] < 7) {
        [self.navigationController.navigationBar setBackgroundColor:[UIColor colorWithRed:0.97 green:0.97 blue:0.98 alpha:1]];
        [self.navigationController.navigationBar setBackgroundImage:[[UIImage alloc]init] forBarMetrics:UIBarMetricsDefault];
    }else{
        [self.navigationController.navigationBar setBarTintColor:[UIColor colorWithRed:0.97 green:0.97 blue:0.98 alpha:1]];
    }
...
- (void)cancel:(id)sender
{
    ...
    {
        self.finishHandle(YES,NO,nil);
    }
}

- (void)done:(NSArray *)assets isCamera:(BOOL)isCamera
{
    [self dismissViewControllerAnimated:YES completion:^{
    ...
    if(self.finishHandle)
    {
        self.finishHandle(NO, isCamera,assets);
    }
}
...
//
    ...
    footView = [[UIView alloc]initWithFrame:CGRectMake(0, 1, self.tableView.frame.size.width, 0.3)];
    if ([[[UIDevice currentDevice] systemVersion]floatValue] < 8.0) {
        footView.backgroundColor = [UIColor colorWithRed:0.78 green:0.78 blue:0.8 alpha:1];
    }else{
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    DPImageSelectVC *vc = [[DPImageSelectVC alloc]init];
    vc.assetsGroup = self.assetsGroups[indexPath.row];
    vc.finishHandle = ^(NSArray *assets){
        [self done:assets isCamera:NO];

    } ;
    
    vc.cameraFinish = ^(UIImage* image){
        [self done:@[image] isCamera:YES];
