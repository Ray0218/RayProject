...
#ifndef __DacaiProject__LotteryJclq__
#define __DacaiProject__LotteryJclq__
...
typedef struct _JclqBetOption {
    int betOptionSf[2];     // 胜负
    int betOptionRfsf[2];   // 让分胜负
    int betOptionSfc[12];   // 胜分差
    int betOptionDxf[2];    // 大小分
} JclqBetOption;

class CLotteryJclqTarget {
public:
    CLotteryJclqTarget();
...
public:
    ...
    JclqBetOption mMixupBetOption;
    JclqBetOption mNormalBetOption;
};

class CLotteryJclqGroup {
public:
    int mGameIdSf;
    int mGameIdRfsf;
    int mGameIdSfc;
    ...
    int mGameIdHt;
    string mGameName;
    vector<CLotteryJclqTarget *> mTargets;
};

class CLotteryJclq : public CModuleBase, public CNotify, public CProjectInfo {
	class Content{
	public:
		Content(){
...
public:
     ...
     *  @param gameName [out]组名 e.g. "2014-08-02"
     *
     *  @return <0表示失败, >=0表示成功
     */
     ...
     *  @param rf              [out]让分(int值, 为实际让分*10)  e.g. -105(-10.5)
     *  @param zf              [out]总分(int值, 为实际总分*10)  e.g. 1555(155.5)
     *
     *  @return <0表示失败, >=0表示成功
     */
    int GetTargetInfo(int groupId, int targetId, string &homeTeamName, string &awayTeamName, string &homeTeamRank, string &awayTeamRank, string &orderNumberName, string &competitionName, string &endTime, int &rf, int &zf);
    /**
     *  获取指定比赛的sp值
     *
     *  @note 客胜索引为0, 主胜索引为1
     ...
     *  @return <0表示失败, >=0表示成功
     */
    int GetTargetSpList(int groupId, int targetId, int spListSf[2], int spListRfsf[2], int spListSfc[12], int spListDxf[2]);
    /**
     *  获取投注选项
     *
     *  @note 客胜索引为0, 主胜索引为1
     ...
     *  @return <0表示失败, >=0表示成功
     */
    int GetTargetOption(int groupId, int targetId, int betOptionSf[2], int betOptionRfsf[2], int betOptionSfc[12], int betOptionDxf[2]);
    /**
     *  设置投注选项
     *
     *  @param groupId  [in]组编号
     ...
     *  @return <0表示失败, >=0表示成功
     */
    int GetFilterInfo(vector<string> &competitions);
    /**
     *  设置筛选条件
     *
     ...
     *  @return <0表示失败, >=0表示成功
     */
    int SetFilterInfo(int *competitions, int length);
...
#pragma mark - 只提供给iOS
     ...
     *  @see GetTargetInfo
     */
    int GetSelectedTargetInfo(int targetId, string &homeTeamName, string &awayTeamName, string &homeTeamRank, string &awayTeamRank, string &orderNumberName, string &competitionName, string &endTime, int &rf, int &zf);
    /**
     *  @see GetTargetSpList
     */
    int GetSelectedTargetSpList(int targetId, int spListSf[2], int spListRfsf[2], int spListSfc[12], int spListDxf[2]);
    /**
     *  @see GetTargetOption
     */
    int GetSelectedTargetOption(int targetId, int betOptionSf[2], int betOptionRfsf[2], int betOptionSfc[12], int betOptionDxf[2]);
    /**
     *  @see SetTargetOption
     */
...
protected:
     ...
     *  @return <0表示失败, >=0表示成功
     */
    int _getTargetInfo(CLotteryJclqTarget *target, string &homeTeamName, string &awayTeamName, string &homeTeamRank, string &awayTeamRank, string &orderNumberName, string &competitionName, string &endTime, int &rf, int &zf);
    /**
     *  获取指定比赛的sp值
     *
     *  @note 客胜索引为0, 主胜索引为1
     ...
     *  @return <0表示失败, >=0表示成功
     */
    int _getTargetSpList(CLotteryJclqTarget *target, int spListSf[2], int spListRfsf[2], int spListSfc[12], int spListDxf[2]);
    /**
     *  获取投注选项
     *
     *  @note 客胜索引为0, 主胜索引为1
     ...
     *  @return <0表示失败, >=0表示成功
     */
    int _getTargetOption(CLotteryJclqTarget *target, int betOptionSf[2], int betOptionRfsf[2], int betOptionSfc[12], int betOptionDxf[2]);
    /**
     *  设置投注选项
     *
     *  @param target   [in]指定的比赛
     ...
     *  @return <0表示失败, >=0表示成功
     */
    int _setTargetOption(CLotteryJclqTarget *target, int gameType, int index, int selected);
     ...
     */
    bool _gameVisible(CLotteryJclqTarget *target);
    /**
     *  清除指定赛事的选项
     *
     ...
     *  @return <0表示失败, >=0表示成功
     */
//    int _cleanupOption(CLotteryJclqTarget *target);
...
public:
	...
	int GetPTarget(int index, int &subNum, string&  OrderNumberName, string& HomeTeam, string &AwayTeam, int &IsDan, string &HomeRank, string&  AwayRank, string &Score, int &Rqs, string &Zf ,string &Rf);
 ...
 protected:
	 ...
	 string Ggfs;
protected:
    vector<CLotteryJclqGroup *> mGroups;
    
    vector<CLotteryJclqGroup *> mAllGroups;
    vector<CLotteryJclqTarget *> mAllTargets;
    ...
    int mSelectedCountSf;
    int mSelectedCountRfsf;
    int mSelectedCountSfc;
    int mSelectedCountDxf;
...
#endif /* defined(__DacaiProject__LotteryJclq__) */
