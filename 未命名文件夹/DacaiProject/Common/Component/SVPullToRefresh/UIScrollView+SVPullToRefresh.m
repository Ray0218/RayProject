...
@property (nonatomic, strong) NSDate *refreshDate;  // 触发刷新的时间, 保证至少有0.2秒的视觉效果
...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler position:(SVPullToRefreshPosition)position {
    ...
    if(!self.pullToRefreshView) {
        ...
        view.delay = 0.3;   // 延时效果(完成时间小于该值时, 将至少停留该值的延时时间)
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.titles = [NSMutableArray arrayWithObjects:@"下拉刷新   ", @"释放刷新   ", @"正在刷新...", nil];
...
- (void)setState:(SVPullToRefreshState)newState {
    ...
    if (newState == SVPullToRefreshStateLoading) {
        self.refreshDate = [NSDate date];
    }
    if (_state == SVPullToRefreshStateLoading && newState == SVPullToRefreshStateStopped) {
        NSTimeInterval delay = [[NSDate date] timeIntervalSinceDate:self.refreshDate];
        if (delay < self.delay) {
...
//        self.arrow.layer.opacity = !hide;
        self.arrow.layer.opacity = self.state != SVPullToRefreshStateLoading;
    }];
}
