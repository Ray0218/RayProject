...
- (void)commonInit
{
    ...
    _contentBlurViewTintColor = [UIColor colorWithWhite:0.7 alpha:0.7];
    
    _contentBlurViewMinAlpha = 0;
    
    _contentBlurViewMaxAlpha = 1.8;
    
    _leftMenuViewVisibleWidth = kScreenWidth;
    
    _rightMenuViewVisibleWidth = kScreenWidth;
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if ([keyPath isEqualToString:@"center"])
    {
        ...
        else if (view == self.rightMenuViewController.view)
        {
            ...
            {
                view.xt_right = CGRectGetWidth(self.view.bounds);
            }
        }
    }
}
...
//                    self.contentBlurView.alpha = self.contentBlurViewMinAlpha + progress * (self.contentBlurViewMaxAlpha - self.contentBlurViewMinAlpha);
                    self.contentBlurView.alpha = (self.contentBlurViewMinAlpha / 2 - self.contentBlurViewMaxAlpha) * pow(progress, 2) + (self.contentBlurViewMaxAlpha * 2 - self.contentBlurViewMinAlpha * 3 / 2) * progress + self.contentBlurViewMinAlpha;
