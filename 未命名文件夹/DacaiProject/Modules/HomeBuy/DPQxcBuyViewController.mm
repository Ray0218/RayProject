...
@interface DPQxcBuyViewController () <UITableViewDelegate, UITableViewDataSource, DPSdBuyCellDelegate, DPDropDownListDelegate, UIGestureRecognizerDelegate> {
@private
    UIView *_drawedView;
...
@implementation DPQxcBuyViewController

- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil];
    if (self) {
        _sevenStarInstance = CFrameWork::GetInstance()->GetSevenStar();
...
- (void)viewDidLoad
{
    ...
    if (self.targetIndex >= 0) {
        int num[70];
        int note;
        _sevenStarInstance->GetTarget(self.targetIndex, num, num + 10, num + 20, num + 30, num + 40, num + 50, num + 60,note);
        memcpy(_qxcNumber, num, sizeof(int) * 70);
    }
...
- (void)pvt_reloadBonusLabel {
    NSMutableAttributedString *bonusString = [[NSMutableAttributedString alloc] initWithString:@"每注2元, 最高奖500万"];
    NSRange numberRange1 = [[bonusString string] rangeOfString:@"2" options:NSCaseInsensitiveSearch];
    NSRange numberRange2 = [[bonusString string] rangeOfString:@"500" options:NSCaseInsensitiveSearch];
    ...
    [bonusString addAttribute:(NSString *)kCTForegroundColorAttributeName value:(id)[[UIColor dp_flatRedColor] CGColor] range:numberRange1];
    ...
    switch (index) {
        case 0: {   // 开奖公告
            viewController = ({
                ...
                viewController.gameType = GameTypeQxc;
        ...
        case 1: {   // 玩法介绍
            viewController = ({
                ...
                viewController.requset = [[NSURLRequest alloc] initWithURL:[NSURL URLWithString:kPlayIntroduceURL(GameTypeQxc)]];
...
- (void)pvt_onSubmit {
    if ([self ballSelectedTotal:_qxcNumber beginIndex:0]<1) {
        [[DPToast makeText:@"第一位至少选择一个号码"]show];
        return;
    }
    if ([self ballSelectedTotal:_qxcNumber  beginIndex:10]<1) {
        [[DPToast makeText:@"第二位至少选择一个号码"]show];
        return;
    }
    if ([self ballSelectedTotal:_qxcNumber beginIndex:20]<1) {
        [[DPToast makeText:@"第三位至少选择一个号码"]show];
        return;
    }
    if ([self ballSelectedTotal:_qxcNumber  beginIndex:30]<1) {
        [[DPToast makeText:@"第四位至少选择一个号码"]show];
        return;
    }
    if ([self ballSelectedTotal:_qxcNumber beginIndex:40]<1) {
        [[DPToast makeText:@"第五位至少选择一个号码"]show];
        return;
    }
    if ([self ballSelectedTotal:_qxcNumber  beginIndex:50]<1) {
        [[DPToast makeText:@"第六位至少选择一个号码"]show];
        return;
    }
    if ([self ballSelectedTotal:_qxcNumber  beginIndex:60]<1) {
        [[DPToast makeText:@"第七位至少选择一个号码"]show];
        return;
    }

    int index=-1;
    if (self.targetIndex<0) {
   index= _sevenStarInstance->AddTarget(_qxcNumber, _qxcNumber + 10, _qxcNumber + 20, _qxcNumber + 30, _qxcNumber + 40, _qxcNumber + 50, _qxcNumber + 60);
    }else{
        index=_sevenStarInstance->ModifyTarget(self.targetIndex,_qxcNumber, _qxcNumber + 10, _qxcNumber + 20, _qxcNumber + 30, _qxcNumber + 40, _qxcNumber + 50, _qxcNumber + 60);
    }
    if (index>=0) {
         [self.navigationController popViewControllerAnimated:YES];
        return;
...
-(void)calculateZhushu{
    int zhushu=_sevenStarInstance->NotesCalculate(_qxcNumber, _qxcNumber + 10, _qxcNumber + 20, _qxcNumber + 30, _qxcNumber + 40, _qxcNumber + 50, _qxcNumber + 60);
...
- (UILabel *)drawedLabel {
    if (_drawedLabel == nil) {
        ...
        _drawedLabel.font = [UIFont dp_systemFontOfSize:13];
...
- (UIView *)controlView {
    if (_controlView == nil) {
        _controlView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 320, 66)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView == self.trendView) {
        ...
        if (cell == nil) {
            cell = [[DPHistoryTendencyCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier digitalType:GameTypeQxc];
        }
        ...
        _sevenStarInstance->GetHistory(indexPath.row, result, gameName)  ;
//        if (_sevenStarInstance->GetHistory(indexPath.row, result, gameName) == 0) {
            cell.gameNameLab.text = [NSString stringWithFormat:@"%d期", gameName];
        NSString * str =[NSString stringWithFormat:@"%d  %d  %d  %d  %d  %d  %d", result[0], result[1], result[2], result[3], result[4], result[5], result[6]];
        if ([str isEqualToString:@"0  0  0  0  0  0  0"]) {
//            str = @"-  -  -  -  -  -  - " ;
            str = @"正在开奖中..." ;

        }
        
        cell.gameInfoLab.text = str ;
...
//        }
    ...
    cell.commentLabel.text = @[@"第一位", @"第二位", @"第三位", @"第四位", @"第五位", @"第六位", @"第七位"][indexPath.row];
    
    for (int i = 0; i < 10; i++) {
        [cell.balls[i] setSelected:_qxcNumber[indexPath.row * 10 + i]];
    }
    
    int miss[70] = { 0 };
    _sevenStarInstance->GetMiss(miss);
    int maxNum = 0 ;
...
- (void)buyCell:(DPSdBuyCell *)cell touchUp:(UIButton *)button {
    ...
    if (button) {
        ...
        _qxcNumber[indexPath.row * 10 + button.tag] = button.selected;
    }
    [self calculateZhushu];
}
#pragma mark- 摇一摇
...
- (void)digitalDataRandom {
    ...
    [self partRandom:1 total:SDNum target2:red];
    for (int i = 0; i < 10; i++) {
        _qxcNumber[i + 10] = red[i];
    }
    
    [self partRandom:1 total:SDNum target2:red];
    for (int i = 0; i < 10; i++) {
        _qxcNumber[i + 20] = red[i];
    }
    [self partRandom:1 total:SDNum target2:red];
    for (int i = 0; i < 10; i++) {
        _qxcNumber[i + 30] = red[i];
    }
    
    [self partRandom:1 total:SDNum target2:red];
    for (int i = 0; i < 10; i++) {
        _qxcNumber[i + 40] = red[i];
    }
    [self partRandom:1 total:SDNum target2:red];
    for (int i = 0; i < 10; i++) {
        _qxcNumber[i + 50] = red[i];
    }

    [self partRandom:1 total:SDNum target2:red];
    for (int i = 0; i < 10; i++) {
        _qxcNumber[i + 60] = red[i];
    }
...
- (void)updateUIData {
    ...
    int results[7] = { 0 };
    int names;
    if (_sevenStarInstance->GetHistory(0, results, names) == 0) {
        self.drawedLabel.text = [NSString stringWithFormat:@"%d %d %d %d %d %d %d", results[0], results[1], results[2], results[3], results[4], results[5], results[6]];
    } else {
    ...
    string endTime;
    int globalSurplus;
    _sevenStarInstance->GetInfo(gameName, endTime, globalSurplus);
...
- (void)refreshNotify {
    ...
    int status = _sevenStarInstance->GetGameStatus(lastGameName, currGameName, endTime);
    if (status != 1) {
        [self updateUIData];
    } else {
        DPLog(@"未获取到数据");
