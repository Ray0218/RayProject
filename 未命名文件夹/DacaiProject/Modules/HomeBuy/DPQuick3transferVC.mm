...
@implementation DPQuick3transferVC
...
- (instancetype)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    if (self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil]) {
         g_nmgksLastGameName = 0;
        self.title = @"快3投注";
        _CQTInstance = CFrameWork::GetInstance()->GetQuickThree();
        self.digitalType=GameTypeNmgks;
        _isSelectedPro=YES;
        [self setTimeSpace:0];
        [self.timer schedule];
        [self.timer fire];
...
-(void)dealloc{
    ...
    _CQTInstance->ClearTarget();
    _CQTInstance->ClearGameInfo();
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    //下拉选一注View隐藏
    self.addyizhuView.hidden = _CQTInstance->GetTargetNum() > 0 ? YES : NO;
    _CQTInstance->SetDelegate(self);
...
- (void)buildLayout {
    ...
    __weak DPQuick3transferVC *weakSelf = self;
    ...
    [self.issureTableView.pullToRefreshView setTextColor:[UIColor dp_flatWhiteColor]];
...
//    [UIBarButtonItem dp_itemWithTitle:@"添加手动" target:self action:@selector(pvt_add)];
...
//            [button setTitleColor:UIColorFromRGB(0x58c854) forState:UIControlStateNormal];
//            [button setBackgroundColor:[UIColor clearColor]];
...
//        });
        UILabel *proLabel = ({
            ...
            [stringM addAttribute:NSForegroundColorAttributeName value:UIColorFromRGB(0x58c854) range:range1];
            ...
            [stringM addAttribute:NSForegroundColorAttributeName value:UIColorFromRGB(0x58c854) range:range3];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return _CQTInstance->GetTargetNum();
}
...
//        if ([NSStringFromClass([vc class]) isEqualToString:@"DPQuick3LotteryVC"]) {
//            DPQuick3LotteryVC *viewControler = (DPQuick3LotteryVC *)vc;
//            [viewControler jumpToSelectPage:indexPath.row gameType:[self buyTypeForOneRow:indexPath.row]];
//        }
//    }
}
...
- (void)touzhuInfotableCell:(DPQuick3DigitalBuyCell *)cell indexPath:(NSIndexPath *)indexPath {
    ...
    int pos;
    int note;
    _CQTInstance->GetTarget(index, content,note, subType, pos);
    NSString *title = @"和值";
    NSString *touzhuInfo = @"";
    NSString *singleString=@"单式";
    int money = content[0] * 2 + content[1] * 10 + content[2] * 50;
    switch (subType) {
        case KSTypeHezhi: {
            title = @"和值";
            touzhuInfo = [NSString stringWithFormat:@"%d", counterNumberTotal[pos / 4][pos % 4]];
            if (money>2) {
                singleString=@"复式";
            }
        ...
        case KSTypeDuizi: {
            ...
            if (pos < 30) {
                touzhuInfo = [NSString stringWithFormat:@"%d", counterNumberSame2[pos / 6][pos % 6]];
                title = @"对子单选";
            } else {
                touzhuInfo = counterNumberSame2More[(pos - 30) % 6];
                title = @"对子复选";
            }
            singleString=@"";
        ...
        case KSTypeBaozi: {
            title = @"豹子单选";
            if (pos < 6) {
                touzhuInfo = [NSString stringWithFormat:@"%d", counterNumberSame3[pos / 3][pos % 3]];
            } else {
                touzhuInfo = @"111/222/333/444/555/666";
                title = @"豹子通选";
            }
            singleString=@"";
        } break;
        case KSTypeShunzi: {
            title = @"顺子单选";
            if (pos < 4) {
                touzhuInfo = [NSString stringWithFormat:@"%d", counterNumberShunzi[pos]];
            } else {
                touzhuInfo = @"123/234/345/456";
                title = @"顺子通选";
            }
            singleString=@"";
        } break;
        case KSTypeErbutong: {
            title = @"二不同号";
            touzhuInfo = [NSString stringWithFormat:@"%d", counterNumberDifferent2[pos]];
            if (money>2) {
                singleString=@"复式";
            }
        } break;
        case KSTypeSanbutong: {
            title = @"三不同号单选";
            if (pos < 20) {
                touzhuInfo = [NSString stringWithFormat:@"%d", counterNumberDifferent3[pos]];
            } else {
                touzhuInfo = @"123/234/345/456";
                title = @"三不同号通选";
            }
    ...
    if (singleString.length>0) {
         cell.zhushuLabel.text= [NSString stringWithFormat:@"%@  %d注  %d元", singleString,money/2,money];
    }else{
       cell.zhushuLabel.text= [NSString stringWithFormat:@"%d注  %d元",money/2,money];
    }
...
- (void)pvt_onBack {
    ...
    if (_CQTInstance->GetTargetNum() <= 0) {
...
-(void)pvt_togetherBuy{
    ...
    CQuickThree *quick3Center = CFrameWork :: GetInstance() -> GetQuickThree();
    int capResult = quick3Center -> CapacityBonusRange(minBonus, maxBonus);
    if (capResult < 0) {
        [[DPToast makeText:@"该方案不支持智能追号"]show];
        return;
    }
    int _amount = 2 * quick3Center -> GetTotalNote();
//    int _gameIsuue ; string _endTime ;
//    quick3Center -> GetInfo(_gameIsuue, _endTime);
    
    int  _periods = 10, _multiple = 1; vector<int>  _szMultiple,_szMmount,_szMinProfit,_szRate;
    ...
    _CQTInstance->CapacityBonusRange(minBonus, maxBonus);
    int totalAmt = _CQTInstance->GetTotalNote() * 2;
    DPLog(@"%d, %d", minBonus - totalAmt, maxBonus - totalAmt);
    
    DPSmartFollowVC *smart = [[DPSmartFollowVC alloc]init];
    smart.gameType = smartZhGameTypeK3;
...
- (void)pvt_submitss {
    [self.addIssueTextField resignFirstResponder];
    [self.addTimesTextField resignFirstResponder];
    int index=_CQTInstance->GetTotalNote();
    if (index<1) {
        [[DPToast makeText:@"请至少选择一注"]show];
        return ;
    }
    if (_CQTInstance->GetTotalNote()<=0) {
    ...
    __block __typeof(_CQTInstance) weakInstance = _CQTInstance;
    void(^block)(void) = ^() {
        int multiple = [weakSelf.addTimesTextField.text integerValue];
        int followCount = [weakSelf.addIssueTextField.text integerValue];
        int money=index*2*multiple*followCount;
        ...
        weakInstance->SetOrderInfo(multiple, followCount, self.afterWinStop);
        CFrameWork::GetInstance()->GetAccount()->SetDelegate(weakSelf);
        CFrameWork::GetInstance()->GetAccount()->RefreshRedPacket(GameTypeNmgks, 1, money, 0);
...
{
    int zhushu = _CQTInstance->GetTotalNote();
    if (zhushu<= 0) {
...
- (void)DPDigitalBuyDelegate:(DPDigitalBuyCell *)cell {
    ...
    _CQTInstance->DelTarget(indexPath.row);
...
-(void)digitalRandomData{
    ...
    int num[KSNUMMAX]={0};
    num[x*3]=1;
    _CQTInstance->AddTarget(num, KSTypeHezhi);
    [self.issureTableView reloadData];
    self.addyizhuView.hidden=_CQTInstance->GetTargetNum()>0?YES:NO;
}
...
-(void)textFieldDidEndEditing:(UITextField *)textField{
    ...
    if ([self.addTimesTextField.text integerValue]>100000) {
        self.addTimesTextField.text=@"100000";
    }
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        switch (cmdType) {
            case ACCOUNT_RED_PACKET: {
                ...
                viewController.gameTypeText = dp_GameTypeFullName(GameTypeNmgks);
...
- (void)alertGameName {
    BOOL showAlert = NO;
       int lastGameName = 0, currGameName = 0; string drawTime;
             DPLog(@"1");
            int ret = CFrameWork::GetInstance()->GetQuickThree()->GetGameStatus(currGameName, drawTime);
            if (ret >=0) {
            if (g_nmgksLastGameName == 0) {
                DPLog(@"111");
                g_nmgksLastGameName = currGameName;
            }
            DPLog(@"3");
            if (g_nmgksLastGameName != currGameName) {
                 lastGameName = g_nmgksLastGameName;
                g_nmgksLastGameName = currGameName;
                showAlert = YES;
                DPLog(@"4");
            }
    ...
    if (showAlert) {
        ...
        NSString *msg = [NSString stringWithFormat:@"尊敬的用户, %@第%02d期已截止, %02d期预售中, 预计开奖时间: %@", gameName, lastGameName, currGameName, time];
...
- (void)pvt_reloadTimer {
    ...
    if ((self.digitalType == GameTypeNmgks) && self.navigationController.viewControllers.lastObject != self) {
...
- (void)setTimeSpace:(NSInteger)timeSpace {
    g_nmgksTimeSpace = timeSpace;
}


- (void)recvRefresh:(BOOL)fromAppdelegate {
    int gameName; string endTime;
    if (_CQTInstance->GetInfo(gameName, endTime) >= 0) {
...
- (BOOL)isBuyController:(UIViewController *)viewController {
    return [viewController isMemberOfClass:[DPQuick3LotteryVC class]];
}
...
- (void)goPayCallback {
    ...
    _CQTInstance->GetWebPayment(buyType, token);
