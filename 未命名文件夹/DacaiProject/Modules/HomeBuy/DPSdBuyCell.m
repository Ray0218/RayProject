...
//        make.bottom.equalTo(contentView);
    ...
    [balls[2] mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(self.ballView);
    }];
    [balls[7] mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(self.ballView);
    }];
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if ((i >= 0 && i < 4) || (i >= 5 && i < 9)) {
            ...
            [ball2 mas_makeConstraints:^(MASConstraintMaker *make) {
                make.left.equalTo(ball1.mas_right).offset(9);
            }];
        }
    }
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    for (NSInteger i = 0 ; i < self.balls.count; i++) {
        ...
        if (CGRectContainsPoint(button.frame, locationCell)) {
            ...
            touchBall.titleText = [NSString stringWithFormat:@"%d", button.tag];
