...
static NSArray *PSTypeNames = @[@"直选", @"组三", @"组六"];

@interface DPPsBuyViewController () <UITableViewDelegate, UITableViewDataSource, DPNavigationMenuDelegate, DPSdBuyCellDelegate, DPDropDownListDelegate, UIGestureRecognizerDelegate> {
@private
...
@property (nonatomic, assign) PSType psType;
@property (nonatomic, assign) BOOL showHistory;
@property(nonatomic,strong)    UILabel *zhushuLabel;//注数
@end

@implementation DPPsBuyViewController

- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil];
    if (self) {
        _pick3Instance = CFrameWork::GetInstance()->GetPick3();
...
- (void)viewDidLoad
{
    ...
    if (self.targetIndex >= 0) {
        ...
        int num[30];
        int note;
        _pick3Instance->GetTarget(self.targetIndex, subType, note,num, num + 10, num + 20);
        
        self.psType = (PSType)subType;
        switch (self.psType) {
            case PSTypeDirect:
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [self.titleMenu setSelectedIndex:self.psType];
    [self.titleButton setTitleText:PSTypeNames[self.psType]];
...
-(void)pvt_onClear{
    for (int i = 0; i < 10; i++) {
        if (self.psType==PSTypeDirect) {
            _direct[i * 3] = 0;
            _direct[i * 3 + 1] = 0;
            _direct[i * 3 + 2] = 0;
        }else if(self.psType==PSTypeGroup6){
            _group6[i] = 0;
        }else if(self.psType==PSTypeGroup3){
            _group3[i] = 0;
        }
    ...
    switch (index) {
        case 0: {   // 开奖公告
            viewController = ({
                ...
                viewController.gameType = GameTypePs;
        ...
        case 1: {   // 玩法介绍
            viewController = ({
                ...
                viewController.requset = [[NSURLRequest alloc] initWithURL:[NSURL URLWithString:kPlayIntroduceURL(GameTypePs)]];
...
- (void)pvt_onSubmit {
    ...
    switch (self.psType) {
        case PSTypeDirect: {
            ...
            if (self.targetIndex<0) {
                index= _pick3Instance->AddTarget(_direct, _direct + 10, _direct + 20, self.psType);
            }else{
                index=_pick3Instance->ModifyTarget(self.targetIndex, _direct,  _direct + 10, _direct + 20, self.psType);
            }
        }
            break;
        case PSTypeGroup6: {
            ...
            if (self.targetIndex<0) {
                index= _pick3Instance->AddTarget(_group6, NULL, NULL, self.psType);
            }
            else{
                index=_pick3Instance->ModifyTarget(self.targetIndex, _group6,NULL, NULL, self.psType);
            }
        }
            break;
        case PSTypeGroup3: {
            ...
            if (self.targetIndex<0) {
                index= _pick3Instance->AddTarget(_group3, NULL, NULL, self.psType);
            }
            else{
                index=_pick3Instance->ModifyTarget(self.targetIndex, _group3,NULL, NULL, self.psType);
            }
        }
            break;
        default:
...
-(void)calculateZhushu{
    ...
    switch (self.psType) {
        case PSTypeDirect: {
            zhushu=_pick3Instance->NotesCalculate(_direct, _direct + 10, _direct + 20, self.psType);
        }
            break;
        case PSTypeGroup3: {
            zhushu=_pick3Instance->NotesCalculate(_group3, NULL, NULL, self.psType);
        }
            break;
        case PSTypeGroup6: {
            zhushu=_pick3Instance->NotesCalculate(_group6, NULL, NULL, self.psType);
        }
            break;
...
- (void)navigationMenu:(DPNavigationMenu *)menu selectedAtIndex:(NSInteger)index {
    [self.titleButton setTitleText:PSTypeNames[index]];
    [self setPsType:(PSType)index];
    [self.tableView reloadData];
    [self calculateZhushu];
    [self pvt_reloadBonusLabel];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView == self.trendView) {
        ...
        int result[3];
        int gameName = 0;
        int ret = _pick3Instance->GetHistory(indexPath.row, result, gameName);
        if (gameName) {
            cell.issueLabel.text = [NSString stringWithFormat:@"%d期", gameName];
            if (ret == 0) {
                cell.drawedLabel.text = [NSString stringWithFormat:@"%d  %d  %d", result[0], result[1], result[2]];
                if (result[0] == result[1] && result[1] == result[2]) {
                    cell.shapeLabel.text = @"豹子";;
                ...
                cell.drawedLabel.text = @"正在开奖...";
                cell.shapeLabel.text = nil;
            }
        } else {
            cell.issueLabel.text = @"-期";
            cell.drawedLabel.text = @"-  -  -";
            cell.shapeLabel.text = nil;
        }
    ...
    _pick3Instance->GetMiss(directMiss, groupMiss);
...
- (void)updateUIData {
    ...
    int globalSurplus;
    int results[3]={0};
    _pick3Instance->GetInfo(gameName, endTime, globalSurplus);
    int ret = _pick3Instance->GetHistory(0, results, gameName);
    if (ret == 0) {
...
- (void)refreshNotify {
    ...
    int status = _pick3Instance->GetGameStatus(lastGameName, currGameName, endTime);
    if (status != 1) {
        [self updateUIData];
    } else {
        DPLog(@"未获取到数据");
