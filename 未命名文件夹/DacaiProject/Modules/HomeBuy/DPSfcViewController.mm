...
@interface DPSfcViewController () <
    DPCollapseTableViewDelegate,
    DPCollapseTableViewDataSource,
    DPScvTabBarViewDelegate,
    ModuleNotify, DPzcBuyCellDelegate,
...
@implementation DPSfcViewController
...
//    self.noDataImgView.hidden = YES;
    ...
    if (self.xt_sideMenuViewController.visibleType == XTSideMenuVisibleTypeLeft) {
        ...
        [self.navigationItem setLeftBarButtonItem:[UIBarButtonItem dp_itemWithImage:dp_NavigationImage(@"home.png") target:self action:@selector(pvt_onNavLeft:)]];
    }
...
- (void)buildLayout {
    ...
    // bind event
    [cleanupButton addTarget:self action:@selector(pvtsfc_onCleanup) forControlEvents:UIControlEventTouchUpInside];
    [confirmButton addTarget:self action:@selector(pvtsfc_onConfirm) forControlEvents:UIControlEventTouchUpInside];
...
-(DPNodataView*)noDataView{
    if (_noDataView == nil) {
        ...
        __block CLotteryZc14 *weak_zc14Instance = _zc14Instance;
        ...
        [_noDataView setClickBlock:^(BOOL setOrUpDate){
            ...
            if (setOrUpDate) {
                ...
                strongSelf->_cmdId = weak_zc14Instance->Refresh();
            }
...
- (void)pvtsfc_onCleanup {
    for (int i = 0; i < 14; i++) {
        _zc14Instance->SetTargetOption(i, 0, 0);
        _zc14Instance->SetTargetOption(i, 1, 0);
        _zc14Instance->SetTargetOption(i, 2, 0);
...
- (int)getSelectedCount {
    ...
    for (int i = 0; i < 14; i++) {
        ...
        int betOption[3] = {0};
        _zc14Instance->GetTargetOption(i, betOption);
...
- (void)pvtsfc_onConfirm {
    int count = [self getSelectedCount];
    if (count < 14) {
        [[DPToast makeText:@"请至少选择14场比赛"] show];
        return;
    }
    DPSfcTransferVC *vc = [[DPSfcTransferVC alloc] init];
    ...
    switch (index) {
        ...
        case 1: { // 玩法介绍
            viewController = ({
                ...
                viewController.requset = [[NSURLRequest alloc] initWithURL:[NSURL URLWithString:kPlayIntroduceURL(GameTypeZc14)]];
...
- (void)showOrChangeBottomText {
    ...
    if(count==0)
    {
        self.promptLabel.text = @"至少选择14场比赛";
        return;
    }
    if (count < 14) {
        self.promptLabel.text = [NSString stringWithFormat:@"选了%d场 还差%d场比赛", count,14-count];
        return;
    }
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    if(_zc14Instance->GetTargetCount()<0){
        return 0;
    }
    return 1;
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    return _zc14Instance->GetTargetCount();
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    int globalSurplus;
    if ((_zc14Instance->GetGameInfo(gameName, onSale, combinationBuyDeadline, globalSurplus)) < 0 ||
        (_zc14Instance->GetTargetCount()) < 0) {
        return nil;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    string combinationBuyDeadline;
    _zc14Instance->GetGameInfo(gameName, onSale, combinationBuyDeadline, globalSurplus) ;
...
- (UITableViewCell *)tableView:(UITableView *)tableView expandCellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cell == nil) {
        ...
        [ cell setClickBlock:^(DPJczqAnalysisCell*nalysisCell){
            ...
            CFrameWork::GetInstance()->GetLotteryZc14()->GetTargetDataInfo(pathIndex.row, competition, matchId) ;
    ...
    [cell clearAllData];
       [self finishDPZc4AnalysisCellData:indexPath cell:cell];
    return cell;
}
...
- (void)finishDPZc4AnalysisCellData:(NSIndexPath *)indexPath cell:(DPJczqAnalysisCell *)cell {
    ...
    _zc14Instance->GetTargetAnalysis(indexPath.row, betProportion, passMatches, count, recentRecord, averageOdds);
...
- (void)tabBarView:(DPSfcTabBarView *)tabBarView selectedAtIndex:(NSInteger)index {
    ...
    _zc14Instance->SetGameIndex(index);
...
-(void)changeHeaderViewWithIndex:(NSInteger)index{
    ...
    NSInteger sectionCount =_zc14Instance->GetTargetCount();
...
- (void)Notify:(int)cmdId result:(int)ret type:(int)cmdtype {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (cmdtype==ANALYSIS) {
            ...
            if (cell) {
                [self finishDPZc4AnalysisCellData:modexIndex cell:cell];
            }
...
//
        ...
        string combinationBuyDeadline;
        int globalSurplus;
       int index1= _zc14Instance->SetGameIndex(0) ;
        _zc14Instance->GetGameInfo(gameName, onSale, combinationBuyDeadline, globalSurplus) ;
        
        int gameNamePre ;
       int index2= _zc14Instance->SetGameIndex(1) ;
        _zc14Instance->GetGameInfo(gameNamePre, onSale, combinationBuyDeadline, globalSurplus) ;
        ...
        if (self.tabBarView.selectedIndex==0) {
             _zc14Instance->SetGameIndex(0) ;
            [self.onTableView reloadData];
            _tableVi = self.onTableView ;
//            self.noDataImgView.hidden = _zc14Instance->GetTargetCount()>0?YES:NO;
        }else{
            _zc14Instance->SetGameIndex(1) ;
            // 重载数据
            [self.preTableView reloadData];
            _tableVi = self.preTableView ;
//            self.noDataImgView.hidden = _zc14Instance->GetTargetCount()>0?YES:NO;
