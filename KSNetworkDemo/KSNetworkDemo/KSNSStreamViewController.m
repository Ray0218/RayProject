...
- (void)viewDidLoad
{
    ...
    self.receiveTextView.textAlignment = NSTextAlignmentLeft ;
}
...
- (void)stream:(NSStream *)stream handleEvent:(NSStreamEvent)eventCode
{
	...
	switch (eventCode) {
		...
		case NSStreamEventHasBytesAvailable: {
           ...
           unsigned int numBytesRead = [(NSInputStream *)stream read:buf maxLength:1024];
		...
		case NSStreamEventEndEncountered: { //达到数据流尾部
